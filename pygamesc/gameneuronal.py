    ...
    while correr:
        ...
        if not pausa:
            ...
            if not modo_auto and not modo_auto_arbol:
                ...
                # Realizar predicciones solo si el jugador está en el suelo
                if en_suelo:
                    ...
                    # Decidir si saltar según la predicción
                    if prediccion[0][0] > 0.3:
                        salto = True
                        en_suelo= False
                        manejar_salto()
            elif modo_auto_arbol and not modo_auto:
                if salto:  # Si ya está en salto, manejarlo
                    manejar_salto()
                if modelo_entrenado_arbol is None:  # Verificar si el Árbol de Decisión está entrenado
                ...
                if en_suelo:
                    ...
                    # Hacer la predicción con el Árbol de Decisión
                    prediccion = modelo_entrenado_arbol.predict_proba(entrada)
                    print(f"Probabilidades del Árbol de Decisión: {prediccion}")

                    # Obtener la probabilidad de la clase "1" (saltará)
                    probabilidad_salto = prediccion[0][1]  # Índice 1 corresponde a la clase positiva
                    print(f"Probabilidad de salto: {probabilidad_salto}")

                    # Decidir si saltar según la probabilidad
                    if probabilidad_salto > 0.3:  # Ajusta el umbral según el comportamiento deseado
                        salto = True
                        en_suelo = False
                        manejar_salto()
